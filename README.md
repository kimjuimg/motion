...
학생이 오늘의 기분을 고르면, 선생님 화면의 목록에 실시간으로 쌓이는 웹앱입니다.
...
- 학생: 배포 주소 접속 → 이름 + **우리 반** 코드 입력 → 감정 선택 → 전달
  (등록된 반 코드를 치면 칸 아래에 `✓ 우리 반: ○학년 ○반` 처럼 반 이름이 바로 떠서
  자기 반이 맞는지 확인할 수 있습니다)
- 선생님: 같은 주소 → **선생님이신가요?** → 교사 코드 입력 → 목록이 실시간으로 채워짐

선생님 화면은 한 줄에 한 명씩 **감정 · 이름 · 반 · 이유 · 시간** 을 보여주는 표입니다.
표 위의 **정렬** 버튼으로 `최신순 / 이름순 / 감정순` 을 바꿀 수 있습니다. 감정순은 같은 감정끼리
묶어 보여주므로 오늘 살펴봐야 할 학생을 찾기 좋습니다.

### 여러 학급 쓰기

선생님 게시판 위쪽 **⚙️ 학급 관리** 에서 학급을 등록합니다. 학급마다 **이름**(예: `○학년 ○반`)과
**코드**(예: `3-2`)를 정하고, 학생에게는 코드를 알려주세요.

**전체** 보기는 기록을 **반별로 나눠서** 보여줍니다. 반마다 이름과 인원수, 그 반의 감정 분포,
그리고 그 반 학생들의 표가 한 덩어리로 묶여 세로로 이어집니다. 등록된 학급이 위에 오고,
그다음은 인원이 많은 순서입니다.

**학급을 등록하지 않아도 반별로 나뉩니다.** 학생이 입력한 코드가 그대로 하나의 묶음이 되고,
`미등록 코드` 라고 표시됩니다. 나중에 **⚙️ 학급 관리**에서 그 코드로 학급을 등록하면 코드 대신
반 이름으로 바뀝니다 — 지난 기록까지 함께 묶입니다.

한 반만 집중해서 보고 싶으면 위쪽 학급 칩을 누르세요. 그 반 기록만 하나의 표로 보입니다.
...
학급 구분은 기록의 `code` 와 `classes` 문서 ID를 대조해서 이루어집니다. 학급을 지워도
기록은 남고, 학급을 다시 만들면 그대로 다시 묶입니다.

학생마다 문서가 따로 생기므로 여러 명이 동시에 제출해도 서로 덮어쓰지 않고,
같은 학생이 다시 제출하면 본인 기록만 바뀝니다. 날짜가 바뀌면 목록은 자동으로 비워집니다.
...
- 익명 로그인이 켜져 있으면 주소를 아는 사람은 누구나 기록을 남길 수 있습니다. 수업 시간에만
  주소를 공유하시고, 필요하면 Firebase 콘솔에서 지난 데이터를 정리하세요.
- **기록 지우기도 같은 칸막이 수준입니다.** 선생님 게시판의 표에서 줄 끝 `✕` → `지울까요?` 로
  기록을 지울 수 있는데, 보안 규칙은 "익명 로그인한 사람"까지만 구분하므로 개발자 도구를 열 줄
  아는 학생도 지울 수 있습니다. 엄격히 막으려면 위와 같이 교사 계정 확인으로 바꿔야 합니다.
