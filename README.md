...
등록하면 목록 위에 학급 칩이 생겨서 **전체 / 3학년 1반 / 3학년 2반** 처럼 골라 볼 수 있습니다.
학급을 고르면 목록과 감정 통계가 그 반 것만 나옵니다. **전체** 보기에서는 표에 **반** 열이
함께 보입니다.

이미 쌓인 기록도 학생이 입력했던 코드로 자동 분류되므로, 나중에 학급을 등록해도 지난 기록이
제자리를 찾아갑니다. 등록되지 않은 코드는 코드 값이 그대로 표시됩니다.
